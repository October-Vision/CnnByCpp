...
/* CNN参数*/
int input_channels = 1;    // 黑白图像，通道为 1
int input_size = 28;       // 图像宽/高
int filter_num = 30;       // 滤波器数量
int filter_size = 5;       // 滤波器尺寸 5x5
int hidden_size = 100;     // 隐藏层神经元数
int output_size = 10;      // 10 分类
...
int main(){
    ...
    //数据预处理
    Tensor x_train_flat = normalize_images(train_dataset.images);
    Tensor x_train = x_train_flat.reshape({60000, 1, 28, 28}); // 重塑为 4D 图像
    Tensor t_train = one_hot_encode(train_dataset.labels);
    int train_size = x_train.shape[0];

    //计算特征图尺寸
    int conv_output_size=(input_size-filter_size)/1+1; // 24
    int pool_output_size=filter_num*conv_output_size*conv_output_size; // 30*24*24 = 17280
    //初始化 CNN 网络参数
    Tensor W1 = Tensor::randn({filter_num, input_channels, filter_size, filter_size}, 0.0f, 0.01f);
    Tensor b1({filter_num}, 0.0f);
    
    Tensor W2 = Tensor::randn({pool_output_size, hidden_size}, 0.0f, 0.01f);
    ...
    //forword
        ...
        //4D_TO2D
        Tensor z1_flat = z1.reshape({batch_size, pool_output_size}); 
        
        Tensor a2 = affine1.forward(z1_flat);     // 输出 2D: (100, 100)
    ...
    //backword
        ...
        Tensor dz1_flat = affine1.backward(dx);   // (100, 17280)
        //展回
        Tensor dz1 = dz1_flat.reshape({batch_size, filter_num, 24, 24});
...
//         int random_test_idx = 42; // 宇宙的终极答案 42
//         cout << "\n[人工抽查验证] 抽取测试集第 " << random_test_idx << " 张图像：\n";
